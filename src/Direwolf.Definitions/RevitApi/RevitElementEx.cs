    ...
    double? WorksetId)
{
    public static RevitElementEx Create(Document doc, ElementId e)
    {
        ...
        if (elementTypeId is null) return new RevitElementEx();
        ...
        return new RevitElementEx(e.Value,
                                  ...
                                  SafeGet(
                                      () => element.AssemblyInstanceId.Value,
                                      -1),
    ...
    private static(string? CategoryName, string? BuiltInCategory)
        HandleCategory(Category c)
    {
